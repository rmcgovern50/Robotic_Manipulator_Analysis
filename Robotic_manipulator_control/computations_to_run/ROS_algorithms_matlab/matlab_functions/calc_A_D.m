function Limits = check_if_admissible(robot_label, x1, x2, qx1_eval, dqx1_eval, ddqx1_eval)
...
Cs = massMatrix(robot_model, single(qx1_evaluated))*transpose(ddqx1_evaluated) + ...
     transpose(velocityProduct(robot_model, single(qx1_evaluated), single(dqdt_evaluated))* (1/x2)); 

gs= transpose(gravityTorque(robot_model, single(qx1_evaluated)));

%% now work out the acceleration limits

ai = (tau_max - Cs*x2^2 - gs)./Ms;
di = (tau_min - Cs*x2^2 - gs)./Ms;

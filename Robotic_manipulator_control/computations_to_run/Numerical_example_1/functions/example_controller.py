...
class path_dynamics_controller():
    ...
    def simulate_trajectory(self, X0, L, direction=1, T=0.01):
        """
        Parameters
        ----------
        state : (x_1, x_2)
    ...
    def scan_input_signs_on_horizontal_axis(self, X0, Ta, number_of_steps=10):
        ...
        x2_points_to_check = list(Ta[-1][1]*np.ones(len(x1_points_to_check)))
    ...
    def find_upper_bound_roa_approx(self, extreme_trajectories):
        
        Td = extreme_trajectories[0]   

        X0=(0.1,15)
        bound_state = self.get_first_boundary_point_upward_scan(X0)
        
        #print(bound_state)
        bound_state = (bound_state[0], 11)
        
        Tu1, _, _ = self.simulate_trajectory(X0, 1,  1, 0.05)
        
        print(len(Tu1))
        
        Tu2, _, _ = self.simulate_trajectory(X0, 0,  1, 0.05)        

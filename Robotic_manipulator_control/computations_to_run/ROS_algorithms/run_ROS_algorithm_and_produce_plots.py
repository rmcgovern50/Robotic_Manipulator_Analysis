...
def find_ROS(situation):
    ...
    #situation 1-4 constraint curve for plotting
    ...
    soft_constraint = combine_to_tuples(x, y_calc)
    
    plotting.plot_constraints(manipulator.boundary_points,\
                              soft_constraint,\
                              current_time,save_file=False)
        
    manipulator.plot_joint_space(ms=1, save_file=False)    
    ###=========================
    
    if situation==1:
        controller_list = sim.situation_1_controller(manipulator)
        #Tu = upper_bound_trajectories
        #flat_Tu = [item for sublist in Tu for item in sublist]
        #Tl = lower_boundary_trajectories
        #flat_Tl = [item for sublist in Tl for item in sublist]
        #print(len(flat_Tu))
        #trajectory_list = [lower_boundary_trajectories, extreme_trajectories[1]]# [Tu[0], Tu[1], Tu[2]]#, flat_Tl]#+ traj_list

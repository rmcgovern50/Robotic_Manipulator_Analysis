...
%C = velocityProduct(ur5,q,qd) %C dq/dt
...
Csx_2x_2 = massMatrix(ur5, single(qx1_evaluated))*transpose(ddqx1_evaluated)*x2_val^2 + ...
     transpose(velocityProduct(ur5, single(qx1_evaluated), single(dqdt_evaluated)));

gs= transpose(gravityTorque(ur5,single(qx1_evaluated)));

u_val = 0.0
tau = (Ms)*u_val +Csx_2x_2 + gs

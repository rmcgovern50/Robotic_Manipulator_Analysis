...
class two_dof_planar_prismatic(pd, pdc):
    ...
    def plot_end_effector_trajectory(self, qs, increment=0.1,\
        ...
        if plot_q1_against_s == 1:
            mv.simple_plot(s_axisq1, 's', 'q1', 1)
        if plot_q2_against_s == 1:
            mv.simple_plot(s_axisq2, 's', 'q2', 1)        
        if plot_q1_against_q2 == 1:
            mv.simple_plot(coordinates, 'q1', 'q2', 1)
        if plot_end_effector_x_y == 1:
            mv.simple_plot(x_y_coordinates, 'x', 'y', 1)
...
class revolute_prismatic(pd, pdc):
    ...
    def dynamics(self):
        ...
        C11 = self.m*self.q2*self.q2d
        C12 = self.m*self.q2*self.q1d
